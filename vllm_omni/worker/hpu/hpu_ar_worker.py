...
class HPUARWorker(HPUWorker):
    """HPU worker for autoregressive omni model stages.

    Extends the base HPUWorker to initialize and manage autoregressive
    model runners for text generation stages (e.g., thinker stages).
    """

    def init_device(self):
        """Initialize the HPU device and autoregressive model runner.

        Sets up CUDA device, initializes distributed environment,
        sets random seed, and creates a HPUARModelRunner instance.

        Raises:
            ValueError: If free HPU memory is insufficient for the
